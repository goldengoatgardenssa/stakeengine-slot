...
export function checkBonus(result) {
    ...
    if (scatters >= GAME_CONFIG.scattersRequired.superBonus) {
        return { type: "SUPER_BONUS", freeSpins: 9, baseMulti: 1.0 };
    }
    if (scatters >= 5) {
        return { type: "BONUS_5_SCATTER", freeSpins: 7, baseMulti: 1.0 };
    }
    if (scatters >= 4) {
        return { type: "BONUS_4_SCATTER", freeSpins: 6, baseMulti: 1.0 };
    }
    if (scatters >= 3) {
        return { type: "BONUS_3_SCATTER", freeSpins: 5, baseMulti: 1.0 };
    }
    return null;
}
